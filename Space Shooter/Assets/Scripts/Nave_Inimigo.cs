...
void FixedUpdate() {
if (transform.position.y < -5) { //Gerência o fim da tela
Destroy(gameObject); //Retira a nave do jogo
//Com o jogo rodando podemos ver a nave sair da aba “Hierarchy”
}    }
void OnTriggerEnter2D(Collider2D other) { //Verifica colisão
if (other.gameObject.CompareTag("Player"))  { //Busca a colisão com a nave do
// jogador
...
if (other.gameObject.CompareTag("Tiro")) { //Gerência se o tiro acertou o inimigo
...
if (vida <= 0) { //Gerência para saber se a nave do inimigo tem vida acima de zero
