...
void Start() {
StartCoroutine(CriaInimigos ()); } //Inicia rotina secundária
IEnumerator CriaInimigos() { //Possibilita ao programa criar pausas entre as
// ondas
yield return new WaitForSeconds(startWave); //Gera a Pausa
while(true){ //Cria um laço infinito enquanto for true=verdadeiro
...
if (proxInimigo <= 15) { //Informa que quando for maior que 15 não será mais
// executado
for (int i = 0; i < waveCount; i++) { // Contador de ondas
...
// inimigo
yield return new WaitForSeconds(tempoWave); } } //Gera a Pausa
...
proxInimigo++; }  }  } //Incrementa valor que gerencia a criação dos
// inimigos