...
public Transform shotSpawn; //Neste campo o SpawnShoot.
...
if (dano > 5 && !timesUp) { // Se o dano for maior que 5 e timesUp for falsa
// ele entra no If
tempoPowerUp = Time.time + 30 ;// tempoPowerUp recebe o tempo atual
// mais 30
timesUp = true; // timesUp recebe verdadeiro
}
if (Time.time > tempoPowerUp) { //Se tempoPowerUp menor que o tempo
...
Instantiate(shot, shotSpawn.position, shotSpawn.rotation);// Instancia shot que 
// é nosso tiro com a posição do SpawnShoot, e rotação do mesmo, como 
// nossa rotação é zero o objeto não tem rotação.

if (shotSpawn != null)
{
    // Faça suas operações aqui
     Debug.Log("está diferente de nulo!!.");
}
}   }   }